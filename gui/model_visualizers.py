...
from .common import VisMode, GradientMode, ShaderBinding, _make_checkerboard, _chw_to_gl_rgba, _hwc_to_gl_rgba, _gradient_to_gl
...
class Gaussian2DVisualizerGUI(VisualizerGUI):
    ...
    def set_gt_image(self, gt_image: torch.Tensor):
        ...
        if self._gt_source_id != new_id:
            ...
            self.gt_image_gl = _chw_to_gl_rgba(gt_image)
    ...
    def prepare_for_display(
        ...
        if vis_mode == VisMode.UPSCALED_TORCH and dx is not None and dy is not None and dxy is not None:
            ...
            result["render"] = _hwc_to_gl_rgba(render_hwc)
            return result
        ...
        rendered_gl = _hwc_to_gl_rgba(render_hwc)

        if vis_mode == VisMode.UPSCALED and dx is not None and dy is not None and dxy is not None:
            result["nearest_src"] = rendered_gl
            result["dx"] = _gradient_to_gl(dx)
            result["dy"] = _gradient_to_gl(dy)
            result["dxy"] = _gradient_to_gl(dxy)
            return result
        ...
        if vis_mode == VisMode.GRADIENTS:
            ...
            if grad is not None:
                result["nearest_src"] = _gradient_to_gl(grad.float())
                return result
...
class LIGVisualizerGUI(Gaussian2DVisualizerGUI):
    ...
    def set_target_image(self, target: torch.Tensor | None):
        ...
        if self._target_source_id != new_id:
            ...
            self.target_image_gl = _chw_to_gl_rgba(target)
