#version 330

smooth in vec2 texcoords;
out vec4 outputColour;

uniform sampler2D texDx;
uniform sampler2D texDy;
uniform sampler2D texLUT;// 256x1 RGB
uniform vec2 pan;
uniform float zoom;
uniform vec2 window_size;
uniform vec2 texture_size;
uniform float scale;// нормализация magnitude, default 2.0

void main()
{
    vec2 screen_pos = texcoords * window_size;
    vec2 center_offset = (window_size - texture_size * zoom) * 0.5;
    vec2 tex_pos = screen_pos - center_offset + pan;
    vec2 uv = tex_pos / (texture_size * zoom);

    if (uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0) {
        outputColour = vec4(0.2, 0.2, 0.2, 1.0);
    } else {
        vec3 dx = texture(texDx, uv).rgb;
        vec3 dy = texture(texDy, uv).rgb;
        vec3 mag = sqrt(dx * dx + dy * dy);
        float avg_mag = (mag.r + mag.g + mag.b) / 3.0;
        float value = clamp(avg_mag * scale, 0.0, 1.0);
        vec3 color = texture(texLUT, vec2(value, 0.5)).rgb;
        outputColour = vec4(color, 1.0);
    }
}
